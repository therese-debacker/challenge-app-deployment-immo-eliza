...
class FeatureEngineering:
    ...
    def remove_outliers(self) -> pd.DataFrame:
        ...
        self.df.drop(self.df[self.df['Property type'] == 'Other_Property'].index, inplace = True)
        self.df.drop(self.df[(self.df['Property type'] == 'Mixed_Use_Building') & (self.df['Living area'] > 1200)].index, inplace = True)
        self.df.drop(self.df[(self.df['Property'] == 'Apartment') & (self.df['Living area'] > 450)].index, inplace = True)
        self.df.drop(self.df[(self.df['Property'] == 'Apartment') & (self.df['Price'] > 1000000)].index, inplace = True)
        self.df.drop(self.df[(self.df['Property'] == 'House') & (self.df['Living area'] > 1200)].index, inplace = True)
    ...
    def replace_navalues(self) -> pd.DataFrame:
        ...
        """
        self.df['Surface of the plot'] = self.df.groupby('district')['Surface of the plot'].transform(lambda x: x.fillna(x.median()))
        self.df['median-price'] = self.df.groupby(['Province', 'Property'])['median-price'].transform(lambda x: x.fillna(x.median()))
        return self.df
